...
class BM25tpRescorer implements Rescorer {
    ...
    public ArrayList<String> getTokens(String text, String factorField, Analyzer analyzer) {
        try {
    ...
    private Map<String, Double> getTokenIDF(long N, Map<String, Integer> queryTokens, String fieldName,
    ...
    private double calcBM25(BM25tpDocInfo docInfo, double boost, Map<String, Integer> tokenCount, float decay) {
        double k1 = 1.2;
        double b = 0.75;
        double K = k1 * (1.0 - b + b * docInfo.termCount / docInfo.avgdl);
        Map<String, Integer> tokens = docInfo.queryTokenCountMap;
        Map<String, Double> idf = docInfo.termIdf;
        Map<String, Double> tf = docInfo.termFreqInDoc;
        double score = 0.0;
    ...
    private double calcTermProximity(BM25tpDocInfo docInfo, double boost, Map<String, Integer> tokenUsedCount,
            float decay) {
        //StringBuilder sb = new StringBuilder();
        ...
        if (docInfo.getTermPosSize() <= 1 || tokens.size() <= 1)
            return 0.0;
        HashMap<String, Double> acc = new HashMap<>();
        for (String s : tokens.keySet()) {
            acc.put(s, 0.0);
        }
        for (int i = 0; i < docInfo.getTermPosSize() - 1; i++) {
            String t1 = docInfo.getTerm(i);
            String t2 = docInfo.getTerm(i+1);
            if (!tokens.containsKey(t1) || !tokens.containsKey(t2))
                continue;
            if (t1 == t2)
                continue;
            int p1 = docInfo.getPos(i); // t1在doc里的位置
            int p2 = docInfo.getPos(i+1); // t2在doc里的位置
            //sb.append(t1).append(":").append(p1).append(" -> ");
            //sb.append(t2).append(":").append(p2).append(" ==>  ");
            for(int j=0;j<docInfo.queryTokenArr.size();j++) {
                if(p2-p1+j >= docInfo.queryTokenArr.size()) 
                    break;
                //sb.append("[")
                //  .append(docInfo.queryTokenArr.get(j))
                //  .append("_pair_")
                //  .append(docInfo.queryTokenArr.get(j+p2-p1))
                //  .append("] ");
                if (docInfo.queryTokenArr.get(j).equals(t1) && 
                    docInfo.queryTokenArr.get(j+p2-p1).equals(t2)) {
                    boost += 0.1;
                    break;
                }
            }
            double dis = (double) (p2 - p1);
            dis = dis * dis;
    ...
    private int getDocInfo(BM25tpDocInfo[] docInfos, IndexSearcher searcher, String factorField,
            Map<String, Integer> termwords) throws IOException {
        ...
        for (int i = 0; i < docInfos.length; i++) {
            ...
            // 需要tf， idf， pos信息
            Terms vector = reader.getTermVector(docInfos[i].docid, factorField); // Read the document's
                                                                                 // document vector.
            ...
            // Document doc = reader.document(docInfos[i].docid);
            // String doc_text = doc.get(factorField);
            // sb.append(" doc_text:").append(doc_text).append("; ");
            // docInfos[i].docString = doc_text;
            ...
            while ((term = terms.next()) != null) {
                ...
                if (termwords.containsKey(termstr)) {
                    ...
                    docInfos[i].termFreqInDoc.put(termstr, (double) freq);
                    ...
                    for (int ii = 0; ii < freq; ii++) {
                        ...
                        docInfos[i].addTermPos(termstr, p);
                    }
                    //sb.append("]");
                    //sb.append(" || ");
                } else {
            ...
            docInfos[i].termCount = termCount;
            docInfos[i].setTermFreqs();
            docInfos[i].setTermPosArr();
            //logger.info(sb);
        }
        return 0;
    }
    ...
    public TopDocs rescore(TopDocs topDocs, IndexSearcher searcher, RescoreContext rescoreContext) throws IOException {
        ...
        //logger.info("doc_num:" + N + " SumDocFreq:" + sumDocFrq + " TotalTermFrq:" +
        // totalTermFrq + " avgdl:" + avgdl
        // + " topdocs.size:" + topDocIds.size());
        int end = Math.min(topDocs.scoreDocs.length, rescoreContext.getWindowSize());
        ArrayList<String> queryTokenArr = getTokens(context.query, context.factorField, context.analyzer);
        Map<String, Integer> queryTokenCountMap = new HashMap<String, Integer>();
        int termWordCount = queryTokenArr.size();
        for(int i=0;i<queryTokenArr.size();i++) {
            String k = queryTokenArr.get(i);
            queryTokenCountMap.put(k, queryTokenCountMap.getOrDefault(k, 0) + 1);
        }
        Map<String, Double> termIdf = getTokenIDF(N, queryTokenCountMap, context.factorField, reader);
        ...
        for (int i = 0; i < end; i++) {
            ...
            docInfos[i].queryTokenArr = queryTokenArr;
            docInfos[i].queryTokenCount = queryTokenArr.size();
            docInfos[i].queryTokenCountMap = queryTokenCountMap;
        ...
        getDocInfo(docInfos, searcher, context.factorField, queryTokenCountMap);
        ...
        // 按照bm25+term_proximity得分排序
        Arrays.sort(docInfos, (a, b) -> {
            if (a.termProximityScore + a.bm25ScoreOriginal > b.termProximityScore + b.bm25ScoreOriginal) {
                return -1;
            }
            return 1;
        });
        ...
        for (int i = 0; i < docInfos.length; i++) {
            if (docInfos[i].termCount * context.minimum_should_match > docInfos[i].termfreqs) {
            ...
            docInfos[i].bm25tpScoreWithDecay = (float) (tp_score_decay + bm25_score_decay); // 取代原有得分
